...
# # Adicionada uma coluna para identificar a empresa
# hapvida['empresa'] = 'Hapvida'
# ibyte['empresa'] = 'Ibyte'
# nagem['empresa'] = 'Nagem'

# # Combinados os datasets
# df = pd.concat([hapvida, ibyte, nagem])

# # Definido o título do painel
# st.title('Painel de Reclamações de Empresas')

# # Filtros dinâmicos solicitados
...
# reclamacoes_por_data = df_filtrado.groupby(df_filtrado['DATA'].dt.date).size()

# st.line_chart(reclamacoes_por_data)
...
# # Frequência de status das reclamações
...
# # Distribuição do tamanho do texto (coluna DESCRICAO)
...
# ax.set_title("Distribuição do Tamanho do Texto")
# st.pyplot(fig)
